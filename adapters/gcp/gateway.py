"""
adapters.gcp.gateway — GCP LLMGateway (WS6).
...
This is the chokepoint that pairs with ``adapters/gcp/egress.yaml`` — the
allow-list declares the same Apigee / Vertex hosts as the only permitted LLM
destinations. Mirrors ``adapters/aws/gateway.AwsLLMGateway`` and
``adapters/azure/gateway.AzureLLMGateway``.
"""
