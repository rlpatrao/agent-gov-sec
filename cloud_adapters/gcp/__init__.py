"""
cloud_adapters.gcp — GCP cloud bindings (WS6).
...
``framework_adapters.langgraph.models.build_gemini_model``.
...
class GcpProvider:
    ...
    def identity_provider(self):
        from cloud_adapters.gcp.identity import GcpIdentityProvider
        return GcpIdentityProvider()

    def trace_exporter_factory(self):
        from cloud_adapters.gcp.tracing import GcpTraceExporterFactory
        return GcpTraceExporterFactory()

    def llm_gateway(self):
        from cloud_adapters.gcp.gateway import GcpLLMGateway
    ...
    async def audit_backend(self, run_id: str):
        from cloud_adapters.gcp.audit import BigQueryHashChainBackend
