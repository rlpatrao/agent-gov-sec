...
policy per (agent, tool) generated by `cedar_export` → a workload identity per
agent → and binds the policy engine to the gateway in **ENFORCE** mode. Live
result: gateway `galaxy-governance-gw`, policy engine `galaxy_governance` with 9
policies, identities `galaxy_{finops,auditor,rogue}`.
