"""
cloud_adapters.aws.gateway — AWS LLMGateway.
...
This is the chokepoint that pairs with ``cloud_adapters/aws/egress.yaml`` — the
allow-list declares the same API-Gateway/Bedrock hosts as the only permitted
LLM destinations. Mirrors ``cloud_adapters/azure/gateway.AzureLLMGateway``.
"""
