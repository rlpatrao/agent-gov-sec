...
framework-neutral ``galaxy_gov.shared.enforcement.pipeline.GuardPipeline``. No governance *logic*
...
``GovernanceViolation`` is defined in ``galaxy_gov.shared.enforcement.pipeline`` and re-exported here
...
from galaxy_gov.shared.enforcement.pipeline import GovernanceViolation, GuardPipeline, build_guard_pipeline
...
async def build_langgraph_governance(
    *,
    ...
    Builds the shared ``GuardPipeline`` (via ``galaxy_gov.shared.enforcement.pipeline.build_guard_pipeline``)
