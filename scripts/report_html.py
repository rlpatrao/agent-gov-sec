...
Consumed by ``scripts/demo_agents.py --html``: takes the default-on matrix
(``CHECKS``) and the flag-gated walk (``demo_extended_guardrails.RESULTS``) and
...
# Default-on controls (identity/egress/per-call guards/FGAC/A2A/reasoning/ledger).
BASELINE_NOTES: dict[str, tuple[str, str]] = {
    "O1": ("Each persona, hosted as its own AgentCore Runtime, calls the governed gateway under its galaxy-rp-<type> identity; Cedar permits/forbids the tool call per agent.",
             "Per-agent authorization is enforced out-of-process at the boundary (FinOps/Auditor allowed, Rogue denied), independent of the agent runtime."),
    "O2": ("tools/list through the AgentCore gateway returns only the tools the agent's Cedar policy permits.",
             "An agent cannot even see tools it is not authorized for (Rogue sees none)."),
...
EXTENDED_META: dict[str, tuple[str, str, str, str]] = {
    ...
    "B4": ("GALAXY_GAP_SEMANTIC_POLICY", "before_tool",
    ...
    "C3": ("GALAXY_GAP_SECURE_CODEGEN", "before_tool",
    ...
    "C4": ("GALAXY_GAP_SECURE_EXEC", "before_tool",
    ...
    "C5": ("GALAXY_GAP_DIFF_POLICY", "before_tool",
    ...
    "N1": ("GALAXY_OPS_SLO_BUDGET", "operational",
    ...
    "N2": ("GALAXY_OPS_ACCURACY_DECL", "operational",
    ...
    "N8": ("GALAXY_GAP_ADVERSARIAL_EVAL", "operational",
...
framework and of the cloud. The <strong>default-on controls</strong> covers identity and
...
CoT/CoVe tracing, and a hash-chained audit ledger. The <strong>flag-gated controls</strong>
...
behaves as the default-on set. Each control below states what it does and why it
exists.\
"""
...
def render_report(
    ...
    *,
    ...
    # ── default-on matrix: input = scenario, output = the recorded actual ──
    base: list[_Row] = []
...
<p class="sub">Unified governance coverage: default-on + flag-gated controls.</p>
...
<div class="cards">
  <div class="card"><div class="n">{b_pass}/{b_total}</div><div class="l">default-on checks · {len(base_codes)} controls</div></div>
  <div class="card"><div class="n">{e_pass}/{e_total}</div><div class="l">flag-gated checks · {len(ext_codes)} controls</div></div>
  <div class="card"><div class="n">{b_pass + e_pass}/{b_total + e_total}</div><div class="l">total checks · {n_controls} controls</div></div>
  <div class="card"><div class="n">{e_intercept}</div><div class="l">flag-gated interceptions</div></div>
</div>
...
<h2>Default-on controls — identity, egress, guards, FGAC, A2A, reasoning, ledger</h2>
<table><thead><tr><th>Code</th><th>Control</th><th>Description</th><th>Input</th><th>Output</th><th>Verdict</th></tr></thead>
<tbody>
{_matrix_rows(base)}
</tbody></table>

<h2>Flag-gated controls — off by default</h2>
