...
def test_renders_unified_report(tmp_path):
    ...
    baseline = [
        ...
        _check("B1 prompt-injection", "Rogue", "block override", True, actual="blocked"),
    ]
    extended = [
        _row("A3", "egress-policy", "WIRED", "evil url", True, True),
        _row("A3", "egress-policy", "WIRED", "allowlisted url", True, False),
        _row("N8", "adversarial-redteam", "OPS", "defense rate", True, True),
    ]
    path = report_html.render_report(
        ...
        baseline_control_map={"A1": "NHI identity", "B1": "Prompt-injection guard"},
...
def test_real_mode_marks_na(tmp_path):
    out = tmp_path / "r.html"
    baseline = [_check("C1 capability", "Rogue", "adversarial tool", False, model_dep=True)]
    report_html.render_report(
        str(out), generated="t", cloud="azure", framework="langgraph", mode="real",
        baseline_checks=baseline, baseline_control_map={"C1": "Capability guard"},
