"""
governance.shared.enforcement.data_fgac — Gap 1: data-layer guardrails (FGAC).
...
class DataAccessMediator:
    ...
    def __init__(
        ...
        drift_detector: Optional[Any] = None,   # governance.shared.enforcement.data_drift.DataAccessDriftDetector
        authorizer: Optional[Any] = None,        # governance.shared.enforcement.policy_engine.CedarAuthorizer (standards-based)
    ) -> None:
        ...
        *,
        ...
        for col in columns:
            ...
            if self._authorizer is not None:
                ...
                # ABACPolicy is keyed by agent_type, so match the evaluator on it
                # (nhi_id is the attributed principal for audit, not policy matching).
                permitted = evaluator.evaluate(agent_id=agent_type, data_label=label).allowed
