"""
governance/remote/registrar.py — the Governance Authority control plane.

The Registrar records ``agent_type → cloud principal`` bindings in the
authority-side :class:`~governance.remote.identity_store.IdentityStore`. It is the
control-plane half of the authority; ``governance.remote.enforce`` is the data
...
1. **Identity enrolled** (this module) — a binding exists and is active. Automated,
   ...
   nothing on its own.
2. **Policy approved** (``governance.policy_export`` → the deployed registry) —
...
from governance.remote.identity_store import IdentityBinding, IdentityStore, IdentityStoreError
