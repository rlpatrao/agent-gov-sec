"""
galaxy_gov/remote/server.py — the Governance Authority service (mechanism 4).
...
``galaxy_gov.remote.enforce`` library — the single enforcement code path. The data
...
Run:
    python -m galaxy_gov.remote.server            # binds 0.0.0.0:8080
    GOV_ENFORCE_PORT=9000 python -m galaxy_gov.remote.server
"""
...
def _caller_identity_from_headers(headers: dict):
    ...
    from galaxy_gov.remote.registrar import CallerIdentity, EnrollmentDenied
...
def _handle_enroll(headers: dict, body: dict) -> tuple[int, dict]:
    ...
    from galaxy_gov.remote.registrar import EnrollmentDenied, enroll
...
def _build_verifier(caller):
    ...
    from galaxy_gov.remote.registrar import EnrollmentDenied, VerifiedPrincipal
    ...
    def do_GET(self):  # noqa: N802
        ...
        if route == "/identity":
            if not _control_authorized(self._headers()):
                ...
                return
            from galaxy_gov.remote.registrar import resolve_identity
