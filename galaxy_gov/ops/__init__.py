"""
galaxy_gov.ops — operational (fleet-level) wrappers over ``agent_sre``.

Unlike ``galaxy_gov.shared.enforcement`` (per-agent-call guards on the GuardPipeline
...
enabled (see ``galaxy_gov.shared.enforcement.flags``).
"""
