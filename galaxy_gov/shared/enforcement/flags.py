"""
galaxy_gov.shared.enforcement.flags — feature flags for the flag-gated guard modules.
...
``galaxy_gov.shared.enforcement`` and ``galaxy_gov.ops``). All default OFF, so an
...
Operational (fleet-level, demo sections in ``galaxy_gov.ops``, not per-call):
