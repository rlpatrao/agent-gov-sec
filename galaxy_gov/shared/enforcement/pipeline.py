"""
galaxy_gov.shared.enforcement.pipeline — the framework-neutral guard pipeline.
...
async def build_guard_pipeline(
    *,
    ...
    logger.info("galaxy_gov.shared.enforcement.pipeline.built",
...
def _register_flag_gated_guards(pipeline: "GuardPipeline", agent_id: str) -> list[str]:
    ...
    if on(flags.EGRESS_POLICY):
        from galaxy_gov.shared.enforcement.egress_guard import EgressGuard
        eg = EgressGuard()
        pipeline.register_before_tool("egress", lambda name, args, _g=eg: _g.check_tool(name, _args(args)))
        wired.append("egress")
    if on(flags.CIRCUIT_BREAKER):
        from galaxy_gov.shared.enforcement.circuit_breaker_guard import CircuitBreakerGuard
        pipeline.set_circuit_breaker(CircuitBreakerGuard())
        wired.append("circuit_breaker")
    if on(flags.SEMANTIC_POLICY):
        from galaxy_gov.shared.enforcement.semantic_policy_guard import SemanticPolicyGuard
    ...
    if on(flags.MEMORY_GUARD):
        from galaxy_gov.shared.enforcement.memory_guard import MemoryWriteGuard
        mg = MemoryWriteGuard()
    ...
    if on(flags.COST_GUARD):
        from galaxy_gov.shared.enforcement.cost_guard import CostGuard
        cg = CostGuard()
