"""
galaxy_gov/tooling/scaffold.py — the `galaxy` developer command.
...
def _enroll(argv: list[str]) -> int:
    ...
    ap.add_argument("--role-prefix", default=os.environ.get("GALAXY_ROLE_PREFIX", "galaxy-rp-"),
                    help="Agent role name prefix. Must match "
                         "galaxy_gov.agentcore.cedar_export.principal_arn (default: galaxy-rp-)")
    ...
    # (galaxy_gov.agentcore.cedar_export.principal_arn) and the Terraform
    ...
    from galaxy_gov.remote.registrar import EnrollmentDenied
...
def _enroll_direct(agent_type: str, principal: str, store_path: str | None, rotate: bool) -> dict:
    ...
    from galaxy_gov.policy_export import export_registry
    from galaxy_gov.remote.identity_store import IdentityStore
    from galaxy_gov.remote.registrar import enroll
...
def _export_registry_cmd(argv: list[str]) -> int:
    ...
    if a.tfvars:
        ...
        # galaxy_gov.agentcore.cedar_export.principal_arn, which lowercases the type.
...
def _verify(argv: list[str]) -> int:
    ...
    from galaxy_gov.policy_export import discover_agent_types, export_registry
    from galaxy_gov.remote.identity_store import IdentityStore
