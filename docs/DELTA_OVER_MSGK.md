...
**Question this answers:** of everything in this repo, what did *we* build versus what comes from the **Microsoft Agent Governance Toolkit (MSGK)** + Microsoft Agent Framework (MAF)?

> **One-liner for an assessor:** *Our delta is **bindings, composition, and attribution — not governance logic.** Every detection/decision primitive (prompt-injection, credential redaction, context budget, policy evaluation, escalation, rogue/anomaly detection, audit interface) is MSGK's. What this repo adds is the **cloud + framework adapter layer**, the **agnostic interface seam** that makes it pluggable, the **per-agent NHI attribution model**, the **hash-chained audit ledger**, the **A2A protocol**, and the **policy/config that drives MSGK's engines** — ~2.7k lines of Python, none of it reimplementing a guard.*

---

## Method / baseline (how this was determined)

The env is `uv`-managed; versions verified from dist-info (WS3): `agent-os-kernel 3.7.0`, `agent-sre 3.7.0`, `agentmesh-platform 3.7.0`, `agent-framework-core/foundry/openai 1.8.1`. The baseline ("what MSGK ships") was established by **introspecting the installed packages' module surface** and mapping it against **every MSGK import in our tree** — both reproducible:
...
# what we import from it:
grep -rnE "(from|import) (agent_os|agentmesh|agent_sre|agent_framework)" core governance a2a adapters payload_agents
```

> No reliable public MSGK *repo* baseline was used — the earlier "v4 umbrella package" GitHub description proved unreliable (the real packages keep their split names). The **installed package surface is the authoritative upstream baseline.**

---

## What MSGK ships vs what we wire

MSGK is large; we wire a thin seam of it. Modules in **bold** are the ones we actually import.
...
- MSGK already ships things on our roadmap: **`agent_sre.anomaly`** is the extension point for **Gap 3** (data-access drift); the **MCP security suite** and **`semantic_policy` / `content_governance`** are unwired surface relevant to **Gap 4**; `agent_framework.{amazon,google}` provider clients are relevant to the **WS5/WS6** framework adapters.

---

## The delta inventory (classified)

Legend: **(a)** pure MSGK pass-through · **(b)** MSGK primitive + our wiring/config · **(c)** wholly ours (no MSGK logic). LOC is our Python.

### (c) Wholly ours — the agnostic seam, bindings, attribution, protocol

| Module | LOC | What it is |
|---|---|---|
| `core/interfaces.py` | 115 | The agnostic **Protocol seam**: `SecretProvider`, `IdentityProvider`, `TraceExporterFactory`, `LLMGateway`, `AgentRuntimeAdapter`, `CloudProvider`. (Re-exports MSGK `AuditBackend` so adapters implement the upstream contract — the one (a) line.) |
...
| `core/nhi_registry.py` | 128 | **Per-agent NHI attribution model** — agent-type → cloud client-id → ledger `nhi_id`. (MSGK has `agentmesh.identity`/`trust`; we don't use it — this binding to cloud IAM is ours.) |
...
| `a2a/envelope.py` | 195 | Typed A2A envelopes (`A2ARequest/Response/Status`). (MSGK ships `agent_framework.a2a`; ours is a purpose-built envelope.) |
| `a2a/dispatcher.py` | 245 | Audited A2A dispatch with trace-linking — *(b)* for audit (uses MSGK `GovernanceAuditLogger`), but the dispatch/envelope protocol is ours. |
...
### (b) MSGK primitive + our wiring/config — the governance composition

| Module | LOC | MSGK primitive wrapped/used | Our part |
|---|---|---|---|
| `adapters/azure/maf/guards/prompt_injection.py` | 125 | `agent_os.prompt_injection.PromptInjectionDetector` | MAF `AgentMiddleware` wrapper + audit emission + **config backfill shim** (MSGK config misses `allowlist`/`blocklist`/`custom_patterns`/`sensitivity`) |
...
| `adapters/azure/maf/middleware.py` | 172 | `agent_os.integrations.maf_adapter.create_governance_middleware` + `ContextScheduler` + `ThreatLevel` | `build_governance_stack()` assembly (guards 1–3 ours, 4–7 from MSGK) + **`_CompatAuditLogger` shim** bridging the kernel-3.x `log()` signature mismatch |
...
- `docs/REFACTOR_AND_GAPS_PLAN.md` — WS1 (the seam/adapters this delta describes), WS3 (the version baseline), WS5/WS6 (AWS/GCP adapters), WS7 (the gaps that build on the unwired MSGK surface).
