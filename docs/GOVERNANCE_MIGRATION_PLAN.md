...
**Already known blocker (from 2026-04-23):** `az login` fails with `SSLCertVerificationError: Basic Constraints of CA cert not marked critical`. IT ticket is open.
...
1. **Where does this code live?** Memory says galaxy-scanner is local-only at `~/Downloads/galaxy-scanner`, no remote. Do we `git init` + push to an org on GitHub/ADO before starting, or continue local until Phase J? Recommendation: push to a private repo before Phase C — Bicep files belong in version control.
