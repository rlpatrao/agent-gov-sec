...
Stack ordering, fail-fast first. Built by [`governance/middleware.py`](../governance/middleware.py) `build_governance_stack()`. Guards 1–3 are this repo's MAF wrappers around MSGK primitives and run before any toolkit middleware; guards 4–7 come from `agent_os.integrations.maf_adapter.create_governance_middleware`. Every guard fires on every `agent.run()` — i.e. on **every governed agent invocation**.

| # | Middleware | MSGK class | Source | OWASP | What it stops |
|---|---|---|---|---|---|
| 1 | `PromptInjectionGuardMiddleware` | wraps `agent_os.prompt_injection.PromptInjectionDetector` | [`governance/guards/prompt_injection.py`](../governance/guards/prompt_injection.py) | ASI-01 / LLM01 | 7-vector taxonomy: direct override, delimiter attack, encoding attack, role play, context manipulation, canary leak, multi-turn escalation. Threat levels NONE / LOW / MEDIUM / HIGH / CRITICAL; blocks at `prompt_injection_block_threshold` (default `medium` in the stack; the Analyzer config sets `high`). |
| 2 | `CredentialRedactorGuardMiddleware` | wraps `agent_os.credential_redactor.CredentialRedactor` | [`governance/guards/credential_redactor.py`](../governance/guards/credential_redactor.py) | LLM06 | API keys, AWS access keys, GitHub tokens, generic secret patterns. Two modes: `redact` (mutate prompt to `[REDACTED]`, proceed — the Analyzer default) or `deny` (block call). |
| 3 | `ContextBudgetGuardMiddleware` | wraps `agent_os.context_budget.ContextScheduler` | [`governance/guards/context_budget.py`](../governance/guards/context_budget.py) | LLM04 | Token-budget allocator + post-call usage record. `context_budget_total_tokens` defaults to 8000 in the stack; the Analyzer config raises it to 40000. |
...
| **Hash-chained ledger** | [`governance/adapters/postgres_audit_backend.py`](../governance/adapters/postgres_audit_backend.py) | Tamper-evident SHA-256 audit chain (stdout/in-memory mode by default; Postgres when `POSTGRES_DSN` is set). |
...
|---|---|---|---|
| `agent_os.egress_policy` | `EgressPolicy`, `EgressRule`, `EgressDecision` | 🟠 Reference-loaded only | Outbound URL allow-list. The `Analyzer` only reaches APIM/AOAI/App Insights/Key Vault — all listed in [`governance/configs/galaxy-egress.yaml`](../governance/configs/galaxy-egress.yaml) and wrapped by [`governance/guards/egress.py`](../governance/guards/egress.py). The minute a tool-using agent that makes outbound HTTP lands, this gets bound to a `FunctionMiddleware` that intercepts every HTTP-shaped tool call. |
...
|---|---|---|
| Hash-chained Postgres audit | [`governance/adapters/postgres_audit_backend.py`](../governance/adapters/postgres_audit_backend.py) | MSGK ships an `audit_logger.AuditBackend` protocol but no concrete SHA-256 hash-chain backend. ~200 LOC fills the compliance-archive gap. (Reconcile against MSGK's Merkle audit trail in WS4.) |
...
### Track 2 — Gap-closing modules (WS7)

Built under `governance/extensions/`, feature-flagged off by default, cloud-neutral with per-cloud adapters. As each lands, move it from this roadmap into the "wired" table above.

| Gap | What's missing today | Plan |
|---|---|---|
| **Gap 1 — Data-layer FGAC** | Governance stops at the IAM + tool boundary. No row/column filtering, no classification-aware masking on what an agent *reads*. | `DataAccessMediator` + data-classification catalog, decisions delegated to MSGK's policy engine; per-cloud FGAC adapters (Purview/SQL CLS, Lake Formation/Macie, BigQuery CLS/DLP). |
| **Gap 2 — Unified policy engine** | ✅ **Adopt upstream — NOT built.** MSGK already ships a standards-based YAML/OPA/Cedar engine. Plan is to adopt it as the single decision point and migrate `galaxy-*.yaml` onto it (a WS3/WS4 config task, not a build). | Confirm engine + syntax in WS3; record adoption in `DELTA_OVER_MSGK.md`. |
| **Gap 3 — Data-access drift** | `RogueAgentDetector` (guard 7) baselines tool-call frequency/entropy but is action-level and the baseline resets on cold start. | Extend with data-access features (volume read, table sensitivity, first-seen-table, read-pattern entropy); persist baselines behind a `BaselineStore` (Postgres/Redis, DynamoDB, Firestore). |
| **Gap 4 — Reasoning-chain guards** | Guards fire at each agent I/O boundary and every A2A hop; `reasoning_tokens` are counted — but intra-LLM reasoning content is never inspected or logged. | (a) **Enforcement:** reasoning-step validator checks intermediate plan/tool-selection steps against the policy engine *before* execution. (b) **Observability (Gap 4+):** capture Chain-of-Thought / Chain-of-Verification, **redact via `CredentialRedactor`/PII before persist**, emit as `reasoning.cot` / `reasoning.cove` span events keyed to `nhi_id`, and write a hash-linked `reasoning_trace` ledger record. |
