...
| 6 | `RogueDetectionMiddleware` | from `agent_os.integrations.maf_adapter` | bundled | Anomaly detection on tool-use patterns. Active for agents that carry tools: Coder (`write_file`, `apply_patch`, `validate_bicep`) and Tester (`run_tests`). Read-only agents (Analyzer, Reviewer, SecurityReviewer) have no tools so this guard is a no-op for them. |
...
| `anomaly` | `AnomalyDetector`, `RogueAgentDetector`, `RiskLevel` | Partially used — `RogueDetectionMiddleware` uses `RogueAgentDetector` for tool-carrying agents (Coder, Tester) | Statistical anomaly detection |
...
If a use case emerges that maps to any 🟠 module, the lift is small (a wrapper + a toggle + a test, mirroring the prompt-injection / credential / context-budget pattern). See the wiring recipe at the end of this doc and the README's "Adding a new agent" section for the full walkthrough.
