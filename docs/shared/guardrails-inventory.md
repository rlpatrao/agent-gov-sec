...
The **`agent_os` + `agent_sre`** packages ship ~40 governance modules. This platform wires a subset. This doc shows exactly which modules the **Galaxy Agentic Governance Platform** wires today, which are available to wire next, the OWASP mapping (with a NIST AI RMF / ISO/IEC 42001 / EU AI Act / MITRE ATLAS crosswalk in [`standards-crosswalk.md`](standards-crosswalk.md)), and how the roadmap aligns with the refactor plan.

> The flag-gated effort wired ~28 additional controls (the previously-unwired
...
article, and MITRE ATLAS by technique name. A control-code view (the demo's A1–L1) is in
...
> **Update (flag-gated controls).** Most modules listed below are now **wired behind a
...
These fire on tool calls that carry the matching shape (code, diff, exec, memory write). Wired behind flags; demonstrated against a code-writing agent shape.

| Module | Class(es) | Status |
|---|---|---|
| `agent_os.reversibility` | `ReversibilityChecker`, `ReversibilityAssessment`, `CompensatingAction`, `ReversibilityLevel` | ✅ Wired (flag `GALAXY_GAP_REVERSIBILITY`) — blocks irreversible actions before they run. Control C6. |
| `agent_os.sandbox` | `ExecutionSandbox`, `SandboxConfig`, `SecurityViolation`, `SandboxImportHook` | ✅ Wired (flag `GALAXY_GAP_SECURE_EXEC`) — `secure_exec.py` validates code in an exec-tool's args. Control C4. |
| `agent_os.memory_guard` | `MemoryEntry`, `Alert`, `AlertType`, `AuditRecord` | ✅ Wired (flag `GALAXY_GAP_MEMORY_GUARD`) — blocks poisoned memory writes. Control G1. |
| `agent_os.diff_policy` | `DiffPolicy`, `DiffFile` | ✅ Wired (flag `GALAXY_GAP_DIFF_POLICY`) — diff/patch size + blocked-path policy. Control C5. |
| `agent_os.secure_codegen` | `CodeSecurityValidator` | ✅ Wired (flag `GALAXY_GAP_SECURE_CODEGEN`) — static review of code in tool args (shell=True, hardcoded secrets, SQL injection, path traversal). Control C3. |
| `agent_os.semantic_policy` | `SemanticPolicyEngine`, `IntentCategory` | ✅ Wired (flag `GALAXY_GAP_SEMANTIC_POLICY`) — intent-classified tool policy (a semantic upgrade of the blocked-pattern substring scan). Control B4. |
| `agent_os.execution_context_policy` | (not inspected) | 🟠 Available — per-agent execution-context isolation. Not wired. |
| `agent_os.constraint_graph` | `ConstraintGraph`, `ConstraintEdge`, `Permission` | ✅ Wired (flag `GALAXY_GAP_CONSTRAINT_GRAPH`) — per-agent deny-by-default constraint graph. Control C7. |
| `agent_os.adversarial` | (re-exports `agent_sre.chaos.adversarial_policy`) | ✅ Wired (flag `GALAXY_GAP_ADVERSARIAL_EVAL`) — red-team harness over `BUILTIN_VECTORS`, drives a guard and reports the defense rate. Control N8. |

### MCP-related — wired behind flags (need an MCP-client agent to exercise live)

`agent_os.mcp_gateway`, `mcp_message_signer`, `mcp_response_scanner`, `mcp_security`, `mcp_session_auth`, `mcp_sliding_rate_limiter`, `mcp_protocols` — seven modules covering MCP tool governance. All wired behind flags: tool gateway (E1), rate limit (E2), response scan (E6), tool-definition screen (E5), session auth (E3), message signing (E4), and the shared audit-sink substrate. See [`extended-guardrails.md`](extended-guardrails.md).
...
| Pydantic+YAML per-agent config | [`galaxy_gov/agent_config.py`](../../galaxy_gov/agent_config.py) | `agent_os` has policy YAML loaders but not per-agent runtime config (`extra="forbid"`). |
...
Two tracks: (1) make the platform cloud-/framework-agnostic; (2) close the four gaps `agent_os` / `agent_sre` do **not** already cover. The old "migration-product roadmap" (more migration agents, per-stack Coder prompts) is **superseded** — that product is archived and not the forward direction.
