...
| `galaxy_gov/guards/egress.py` | 55 | `agent_os.egress_policy.EgressPolicy` | guard wrapper + allow-list path via the provider factory + **`protocol: tcp` parser workaround** |
| `galaxy_gov/guards/escalation.py` | 88 | `agent_os.escalation.*` | guard wrapper + audit |
