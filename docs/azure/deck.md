...
theme: default
paginate: true
header: 'Galaxy Agentic Governance Platform — Azure'
style: |
...
Runtime governance and security for multi-agent systems on Azure —
API Management and the Microsoft Agent Framework. Built on the Microsoft Agent
Governance Toolkit (`agent_os` · `agent_sre` · `agentmesh`).
...
Azure infrastructure, execution flows, demo results, glossary. Audience is engineering.
-->
...
- Per-agent identity: a Non-Human Identity (NHI) bound to an Entra Managed Identity.
...
- OpenTelemetry tracing: framework-neutral spans exported to Azure Monitor / Application Insights.
- A hash-chained audit ledger: a tamper-evident SHA-256 chain in PostgreSQL.
...
1. **LLM-agnostic** — the model is reached only through a gateway; deployment id injected server-side.
2. **Agent-framework-agnostic** — the same governance wraps MAF, LangGraph, a raw loop, and Pydantic AI.
...
- Light blue (wired and composed): the Guard Library, Fleet Ops, and the FGAC
  enforcement — on Azure through the MAF middleware stack. No detectors were reimplemented.
...
| Element | Status |
|---|---|
| In-process `GuardPipeline` + MAF middleware stack + demo matrix | **Live** — offline + live Azure OpenAI runs (default cloud) |
...
| MAF hosting on Azure AI Foundry Agent Service (Method 2) | **Reference** — runtime adapter built |
...
## Guard catalogue — 47 controls · 84 checks
...
| Data access | mediator | FGAC — mask · row-filter · Azure SQL / Synapse pushdown · deny |
...
## All 47 controls — 14 categories, per-category numbering
...
47 controls (21 On · 26 Flag) · 84 checks. Each wraps an `agent_os` / `agent_sre` primitive
— full catalogue in `architecture.md` §3.2. Authorization on Azure is in-process (the MAF
policy / capability / A2A middlewares) plus the APIM edge — there is no managed Cedar engine
as on AWS, so no category O.
...
## Galaxy on the Azure reference architecture

Full grid: **`reference-architecture-azure.html`** (import as a slide image).
...
- The enforcement chokepoints in Layer 04: the Function proxies and the data proxy.
...
Service, AKS/Container Apps, VNet, Key Vault). Authorization is done in-process by the MAF
policy middleware and re-checked at the APIM edge; `agent_os` supplies the detectors;
Galaxy composes them.
...
![w:760 center](../diagrams/arch-stack-azure.svg)
...
The `GuardPipeline` (4 hooks plus the guard library), the MAF middleware stack, the floor, the core seam, and A2A dispatch.
...
|---|---|---|
| Path | APIM → `llm_proxy` Function → Azure OpenAI | MAF agent (governance middleware) on Foundry / Container Apps |
| Authorization | policy registry → fail-closed 403 + APIM edge | in-process MAF policy / capability / rogue middlewares |
| Content controls | `llm_proxy.enforce_llm` (input · tool-plan · output) | MAF guard middlewares (`maf/guards/`) |
| Identity | Entra Managed Identity `galaxy-<persona>-mi` | same Managed Identity, bound to the MAF host |
| Provisioning | Bicep `main.bicep` (reference) | MAF host + `aca_jobs.bicep` (reference) |

Neither option lets the agent hold the Azure OpenAI key; it is injected at the APIM edge and
the deployment id is pinned server-side.
...
## Azure infrastructure — both options

![w:900 center](../diagrams/azure-deploy-options.svg)

**Method 1:** APIM `galaxy-<suffix>-apim` → Function `galaxy-<suffix>-func` (`enforce_llm`) →
Azure OpenAI. **Method 2:** MAF agents with the governance middleware stack on Azure AI
Foundry Agent Service / Container Apps. Shared: Azure OpenAI, PostgreSQL `trace_ledger`,
Entra Managed Identities `galaxy-<persona>-mi`, Key Vault, Application Insights. Full layered
...
The same `EnforcementSession` runs in-process and again at the Function boundary. The
boundary check is authoritative, and the Function reads its Azure OpenAI key from Key Vault
(or receives it injected at the APIM edge), so the agent never holds it.

---

## Method 2 — MAF enforcement flow

![w:840 center](../diagrams/maf-infra-azure.svg)

MAF agent → guard middlewares (prompt-injection → credential → context budget → policy →
capability → rogue) → model → output redaction. Every decision is written to the Postgres
ledger and emitted as an Application Insights span.
...
Three personas, exercised on Azure and all frameworks:
...
```bash
.venv/bin/python scripts/demo_agents.py --azure --extended       # live Azure OpenAI
...
A self-contained, self-describing table (control · input · output · verdict) is produced by
`--html`. Verdicts are `PASS`, `N/A` (model discretion), or `FAIL` (non-zero exit).
...
| Term | Meaning |
|---|---|
| **NHI** | Non-Human Identity — per-agent Entra Managed Identity `galaxy-<persona>-mi` |
...
| **MAF** | Microsoft Agent Framework — the Azure-native agent framework; guards run as middlewares |
| **APIM** | Azure API Management — the managed LLM-egress edge (Method 1) |
| **A2A** | agent-to-agent — typed envelopes + audited dispatch |
| **Hash-chained ledger** | tamper-evident SHA-256 chain in PostgreSQL `trace_ledger` |
| **App Insights** | Application Insights — OTel span sink; `gen_ai.*` spans to the "Agents (preview)" dashboard |
