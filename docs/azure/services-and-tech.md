...
- **Governance audit attributes** (emitted by `OtelAuditBackend` as *span events* named `governance.<event_type>`, not span attributes — [galaxy_gov/adapters/otel_audit_backend.py](../../galaxy_gov/adapters/otel_audit_backend.py)): `governance.agent_id` (NHI principal), `governance.event_type`, `governance.action`, `governance.decision`, `governance.reason`, `governance.latency_ms`, plus arbitrary scalar metadata as `governance.metadata.<key>`.
