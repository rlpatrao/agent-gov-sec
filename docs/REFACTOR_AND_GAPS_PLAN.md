...
>
> **MSGK reconciliation + Cedar — ✅ done (2026-06-10):** prompted by a policy-engine review, verified `agent_os.policies` is a full ABAC engine (native conditions + **Cedar/OPA backends** + a `data_classification` module). **Gap 1 refactored to consume MSGK's `agent_os.policies.data_classification`** (`DataClassification`/`DataLabel`/`ABACPolicy`/`DataAccessEvaluator`) for the decision — only the enforcement + config catalog stay ours (shrinks our delta). **Cedar wired** as the standards-based engine for agent + data authz: [`governance/extensions/policy_engine.CedarAuthorizer`](../governance/extensions/policy_engine.py) over MSGK's `load_cedar`, example [`configs/authz.cedar`](../governance/extensions/configs/authz.cedar), flag `GALAXY_POLICY_ENGINE=cedar`, `.[cedar]` extra (`cedarpy`). Casbin evaluated and rejected (would be a redundant third engine). This also realizes part of **Gap 2** (standards-based engine adopted, not built).
>
> **Still deferred:** 7.1.4 GCP (BigQuery CLS/DLP) + Azure (Synapse CLS) pushdown adapters; 7.4.3 semantic CoT analysis (research-y); 7.5.6 Azure/GCP query parity; full Cedar conditional eval in CI needs `cedarpy` installed. None block the modules from functioning.
