...
**Acceptance:** ✅ `CLOUD_PROVIDER=aws` resolves all cloud-binding interfaces with real impls + tests; Terraform reference (tagged `galaxy-rp`) `validate`s; the `apigw-bedrock` live model + Lambda proxy are wired and unit-tested; `--aws --fake` → 37/37; framework adapter status documented (5.8 deferred). ⚠️ **Runtime-verified vs stubbed:** the offline paths (fake-model matrix, Converse mapping, gateway fallback) are tested here; **live AWS is NOT yet exercised** — it needs the AWS CLI + account creds, `terraform apply` of the `galaxy-rp` infra, and Bedrock model access enabled for Claude 3.5 Sonnet in us-east-1. Run `demo_agents.py --aws` after applying to verify the real Bedrock path end-to-end.
