...
A runtime, **framework-agnostic** governance & security platform for multi-agent systems, built on the open-source `agent_os`, `agent_sre`, and `agentmesh` packages (the Microsoft Agent Governance Toolkit). It governs agents from **any** framework through thin adapters over a single, framework-neutral [`GuardPipeline`](governance/pipeline.py) — **LangGraph** (full demo) and **Pydantic AI** (`adapters/pydantic_ai/`, governance-parity tests) today, a **raw** provider-native loop planned — providing per-agent identity, a layered guard stack, A2A governance, OTel tracing, and a hash-chained audit ledger, independent of the agents it governs.
