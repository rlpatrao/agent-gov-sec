...
A runtime, **framework-agnostic** governance & security platform for multi-agent systems, built on the open-source `agent_os`, `agent_sre`, and `agentmesh` packages (the Microsoft Agent Governance Toolkit). It governs agents from **any** framework through thin adapters over a single, framework-neutral [`GuardPipeline`](governance/pipeline.py) — **LangGraph**, **Pydantic AI**, and an unopinionated **raw** provider-native loop, all running the full demo matrix (`--framework {langgraph,raw,pydantic}`, each 37/37) with identical governance — providing per-agent identity, a layered guard stack, A2A governance, OTel tracing, and a hash-chained audit ledger, independent of the agents it governs.
