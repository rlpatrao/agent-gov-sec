...
3. Register the NHI: add a `NHI_CLIENT_ID_<NAME>` default in `payload_agents/__init__.py` and the same key to `.env.example`.
4. Create `payload_agents/config/<name>.yaml` + `payload_agents/prompts/<name>.md` (the Pydantic schema enforces `extra="forbid"`).
5. Add tests to `tests/test_<framework>_*.py`.

See [`docs/shared/adding-an-agent.md`](docs/shared/adding-an-agent.md) for the developer/governing-team split and [`docs/aws/user-guide.md`](docs/aws/user-guide.md) for the full walkthrough.
